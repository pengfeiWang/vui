...
function filterNext (direction) {
  let tmp = [];
  let isQuery = this.selects[this.keyIndex] ? this.queryItem(this.selects[this.keyIndex][this.keyLabel]) : true;
  let len = this.selects.length;
  let isHidden = this.hiddens.indexOf(this.keyIndex);

  if (this.create && this.isFilter && this.value && this.isShowNewLabel) {
    len += 1;
  }
  ...
  // if (this.value && this.create) {
  //   tmp = this.selects.find((it, idx) => {
  //     if (it[this.keyLabel] === this.value) {
  //       return true;
  //     }
  //   });
  //   if (!tmp) {
  ...
  if ((this.create || this.filter) && this.keyIndex !== '') {
    ...
    if (this.keyIndex === this.selects.length && this.searchValue && 
      ...
      if (this.multiple) {
        ...
        this.newOptions.splice(0, 1, o);
        this.tagsList.splice(0, 1, o);
      }
    }
  }
}
...
export default {
  ...
  computed: {
    ...
    wrapClasses () {
      ...
      return {
        ...
        [`${this.prefixCls}-select-disabled`]: this.disabled,
    ...
    selectionClasses () {
      return {
        ...
        [`${this.prefixCls}-select-selection-create`]: this.create || this.filter
      };
    },
    ...
    isFilter () {
      return this.create || this.filter;
    },
  ...
  methods: {
    ...
    handleChange () {
      ...
      if (create && !isInsert /*&& isTags*/ && value) {
        this.$nextTick(() => {
          ...
          /**
           * 根据当前输入内容查找到传进来的selects集合中对应的index
           */
          labels.find((it, idx) => {
            if (reg.test(it)) {
              index = idx;
    ...
    handleEnter (event) {
      ...
      // 启用过滤, 未开启创建, 同时value 为真, 并在上下翻页
      if (this.filter && !this.create) {
        if (!this.selects.find(it => {return this.queryItem(it[this.keyLabel]);})) {
          this.keyIndex = '';
        }
      }      
      // 判断条件不一定是value, 有选中的时候回车事件也需要继续执行
      // if (!this.value) {
      //   return;
      // }
      if (this.create) {
        createNewOption.call(this);
      }
    ...
    handleMultipleChange (...ags) {
      ...
      let obj = {
        el: this.$refs.reference,
        'el-item': this.$refs.itembox,
        'el-input': this.$refs.input,
        'el-rendered': this.$refs.rendered,
        selected: false
      };
      // 多选首先要清空 选中的 label
      ...
      if (item) {
        ...
        obj.selected = true;
        oEv.target = this.$refs.itembox.children[this.keyIndex];
        oEv.type = 'create';
      }
      if (this.value) {
        this.value = '';
      ...
      obj.item = item;
      obj.index = index;
      obj.selecteds = this.selecteds;
      obj.newOptions = this.newOptions;
      obj.ev = oEv;
    ...
    defaultSelectItem (it) {
      ...
      //   return cls;
      // }
      if (this.selectValue.indexOf(it[this.keyValue]) > -1) {
        return cls;
      }
      return '';
    },
    ...
    navigateOptions (direction) {
      ...
      // 启用过滤, 未开启创建, 同时value 为真, 并在上下翻页
      if (this.filter && !this.create && (direction === 'next' || direction === 'prev')) {
        if (!this.selects.find(it => {return this.queryItem(it[this.keyLabel]);})) {
          return;
        }
      }
      ...
      if (create && isFilter && value && this.isShowNewLabel) {
        len += 1;
      }
      // if (this.value && this.create) {
      //   if (this.filter && !this.selects.find(it => {return this.queryItem(it[this.keyLabel]);})) {
      //     tmp = this.selects.find((it, idx) => {
      //       if (it[this.keyLabel] === this.value) {
      //         return true;
      //       }
      ...
      // }
      if (!this.isTags && (direction === 'left' || direction === 'right')) {
        return;
      }
    ...
    setSize () {
      ...
      if (!v && !this.tagsList.length) {
        ...
        this.$nextTick(() => {
          if (this.$refs.input && 
            this.$refs.search__field__mirror && this.isFilter) {
          ...
          // .getBoundingClientRect().width;
            if (renderedW <= iW) {
              ...
              this.cpInputWidth = this.value.length <= 1 ? this.minWidth : iW + 3 + 'px';
            }
          } else {
    ...
    initial () {
      ...
      if ((!this.filter || !this.create) && !this.value) {
        this.minWidth = '3px';
      }
      this.handleLabel();
      this.setSize();
